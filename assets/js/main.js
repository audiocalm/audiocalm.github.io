...
(function () {
  ...
  /* ---------- render one sample card (compact: GT + OURS + featured + button) --- */
  function renderSample(sample, taskData, taskKey) {
    const models = (taskData && taskData.models) || [];
    const featured = (taskData && Array.isArray(taskData.featured)) ? taskData.featured : [];
    const card = el("div", { class: "sample-card" });

    // header — sample label (dataset moved to section title)
    ...
    if (sample.prompt !== undefined) {
      ...
      promptRow.appendChild(
        el("span", { class: "label", text: "Speaker prompt" })
      );
    ...
    // featured baselines (one per task, curated externally — no labels here)
    featured.forEach((key) => {
      const m = models.find((mm) => mm.key === key);
      if (!m || m.ours) return;
      const p = sample.audios ? sample.audios[m.key] : "";
      rows.appendChild(buildRow(m.name, null, p, "baseline"));
    });
  ...
  function openCompareModal(sample, models, taskKey) {
    ...
    header.appendChild(
      el("div", { class: "modal-eyebrow", text: taskLabel + (taskLabel ? " · " : "") + label })
    );
    if (sample.text) {
    ...
    if (sample.prompt !== undefined) {
      ...
      promptRow.appendChild(el("span", { class: "label", text: "Speaker prompt" }));
      const p = audioPlayer(sample.prompt);
      promptRow.appendChild(p.node);
      if (!p.present) {
    ...
    // Ground Truth on top (only when available)
    if (sample.gt) {
      body.appendChild(
  ...
  function mount(containerId, taskData, taskKey) {
    ...
    taskData.samples.forEach((s) =>
      root.appendChild(renderSample(s, taskData, taskKey))
    );
  }
